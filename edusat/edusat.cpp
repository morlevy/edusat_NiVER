...
// Function to check for tautology in a sorted clause
...
void Solver::fixCnf() {
    vector<Clause> niverClauses = cnf;
    //remove duplicates
    sort(niverClauses.begin(), niverClauses.end());
    niverClauses.erase(unique(niverClauses.begin(), niverClauses.end()), niverClauses.end());
    set_nclauses(niverClauses.size());
    initialize();
    reset();

    cnf.clear();
    for (auto& clause : niverClauses) {
        if (clause.size() == 0)
            continue;
        else {
            if (clause.size() == 1) {
...
size_t union_size(const vector<Clause>& pos_clauses, const vector<Clause>& neg_clauses){
    std::vector<Clause> united;
    std::set_union(
            pos_clauses.begin(), pos_clauses.end(),
            neg_clauses.begin(), neg_clauses.end(),
            std::back_inserter(united));
    // return num of literals in united
    size_t size = 0;
    for (const auto& clause : united) {
        size += clause.size();
    }
    return size;
}

void separateClauses(const std::vector<Clause>& cnf, int var, std::vector<Clause>& cnfWithoutVar, std::vector<Clause>& cnfWithVar) {
    for (const auto& clause : cnf) {
        if (clause.exists(v2l(var)) || clause.exists(v2l(-var))) {
            cnfWithVar.push_back(clause);
        } else {
            cnfWithoutVar.push_back(clause);
        }
    }
}


void removeClause(std::vector<Clause>& cnf, const Clause& clause) {
    cnf.erase(std::remove(cnf.begin(), cnf.end(), clause), cnf.end());
}

// Function to resolve two clauses on a variable
std::vector<int> resolve(const Clause& pos_clause,const Clause& neg_clause, int var) {
    std::vector<int> resolvent;
    std::set_union(
            pos_clause.begin(), pos_clause.end(),
            neg_clause.begin(), neg_clause.end(),
            std::back_inserter(resolvent)
    );
    resolvent.erase(std::remove(resolvent.begin(), resolvent.end(), v2l(var)), resolvent.end());
    resolvent.erase(std::remove(resolvent.begin(), resolvent.end(), v2l(-var)), resolvent.end());
    // sort the resolvent to allow duplicate checking
    std::sort(resolvent.begin(), resolvent.end());
    return resolvent;
}

// Apply NiVER preprocessing to the CNF
void Solver::applyNiVER() {
    bool entry = true;
    while (entry) {
        entry = false;
        size_t old_literals = 0;
        // Try to eliminate each variable
        for (int var = 1; var <= nvars; var++) {
            vector<Clause> pos_clauses, neg_clauses;
            std::set<std::vector<Lit>> resolvents; // unordered set is better

            for (const auto& clause : cnf) {
                int pos = v2l(var), neg = v2l(-var);
                if (clause.exists(pos)) {
                    pos_clauses.push_back(clause);
                }
                if (clause.exists(neg)) {
                    neg_clauses.push_back(clause);
                }
            }

            size_t new_literals = 0;

            // Calculate old literals count
            old_literals = union_size(pos_clauses, neg_clauses);

            vector<Clause> cnfWithoutVar, cnfWithVar;
            //separateClauses(cnf, var, cnfWithoutVar, cnfWithVar);

            std::sort(cnf.begin(), cnf.end());
            std::sort(pos_clauses.begin(), pos_clauses.end());
            std::sort(neg_clauses.begin(), neg_clauses.end());

            set_union(
                    pos_clauses.begin(), pos_clauses.end(),
                    neg_clauses.begin(), neg_clauses.end(),
                    std::back_inserter(cnfWithVar)
                    );

            set_difference(
                    cnf.begin(), cnf.end(),
                    cnfWithVar.begin(), cnfWithVar.end(),
                    std::back_inserter(cnfWithoutVar)
                    );

            
            for (auto& pos_clause : pos_clauses) {
                for (auto& neg_clause : neg_clauses) {
                    std::vector<Lit> resolvent = resolve(pos_clause, neg_clause, var);
                    if (!is_tautology(resolvent)) {
                        if (resolvents.count(resolvent) || resolvent.size() == 0) {
                            continue;
                        }
                        resolvents.insert(resolvent);
                        new_literals += resolvent.size();
                    }
                    // If no increase in literals, eliminate the variable
                    if (new_literals <= old_literals) {
                        removeClause(cnfWithVar, pos_clause);
                        removeClause(cnfWithVar, neg_clause);
                        entry = true;
                        cnf = cnfWithoutVar;
                        cnf.emplace_back(Clause(resolvent));
                        cnfWithoutVar = cnf;
                    }
                }
            }
            for (auto clause: cnfWithVar) {
                if (clause.size() == 1){
                    unaries.push_back(clause.cl()[0]);
                    continue;
                }
                // duplicate checking TODO optimize
                niverUncheckedClauses.push_back(clause);
            }
        }
    }
    fixCnf();
...
void Solver::initialize() {
    ...
    for (unsigned int i = 0; i < nlits + 1; i++) {
        watches[i].clear();
    }
	//reset();
}
...
void Solver::validate_assignment() {
    ...
    for (vector<Clause>::iterator it = beforeNiverClauses.begin(); it != beforeNiverClauses.end(); ++it) {
        int found = 0;
        for(clause_it it_c = it->cl().begin(); it_c != it->cl().end() && !found; ++it_c)
            if (lit_state(*it_c) == LitState::L_SAT) found = 1;
        if (!found) {
            for (clause_it it_c = it->cl().begin(); it_c != it->cl().end() && !found; ++it_c)
                if (lit_state(*it_c) == LitState::L_UNASSIGNED){
                    if (*it_c %  2 == 0){
                        state[l2v(*it_c)] = VarState::V_TRUE;
                    }
                    else{
                        state[l2v(*it_c)] = VarState::V_FALSE;
                    }
                    break;
                }
        }
    }
    //cnf.insert(cnf.end(), niverUncheckedClauses.begin(), niverUncheckedClauses.end());
	for (vector<Clause>::iterator it = beforeNiverClauses.begin(); it != beforeNiverClauses.end(); ++it) {
		int found = 0;
		for(clause_it it_c = it->cl().begin(); it_c != it->cl().end() && !found; ++it_c) 
			if (lit_state(*it_c) == LitState::L_SAT) found = 1;
		if (!found) {
			...
			cout << endl;
			Abort("UNSAT: Assignment validation failed", 3);
		}
	}
...
SolverState Solver::_solve() {
	...
	while (true) {
		...
		if (res == SolverState::SAT){
            /**if (niverUncheckedClauses.size() > 0) {
                auto clause = niverUncheckedClauses.back();
                add_clause(clause, 0, clause.size() - 1);
                niverUncheckedClauses.pop_back();
                backtrack(analyze(cnf.back())); // TODO change, I'm not sure how.
                goto DECIDE;
            }**/
            return res;
        }
